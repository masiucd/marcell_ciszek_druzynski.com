...
Take the quiz-box below to test your knowledge.
...
console.log(proto === ronaldo.__proto__) // true
```
...
It's important to note that when creating constructor functions and generating new instances with the `new` keyword, arrow functions in JavaScript will not work as expected. Arrow functions do not have their own this context, which means they do not require a prototype property. If you attempt to use an arrow function as a constructor, you will encounter a TypeError with a message similar to _Person is not a constructor._
