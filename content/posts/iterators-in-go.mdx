...
## Exciting News: Iterators in Go Language

We finally have iterators in the Go programming language, and I couldn't be more excited about it! If you're unfamiliar with what an iterator is, it's a design pattern that many other languages, such as JavaScript, Java, and Python, have been using for some time.
...
This overview provides insight into what you can do with iterators and their general functionality. While iterators are not yet fully integrated into the Go language.
...
We got, *Single-value iterators `iter.Seq`* and *Two-value iterators: `iter.Seq2`*. With these two new types it will be easier for us to describe what our iterator return type is.
...
I appreciate how the Go specification introduces new features with a cautious perspective. By allowing developers to create their own implementations initially, the community can evaluate whether there is a genuine demand for these features.
...
At the outset, it's likely that the Go community will implement custom solutions similar to what we see with libraries like *[Lodash](https://lodash.com/)* in JavaScript. A third-party library could emerge to provide higher-order functions such as `map`, `filter`, and `reduce`.
...
Right now there are arguments that it is not readable and brings a lot of complexity and I am agree. But I do think when devs has got used to the iterators and we have a more common way to use them together with usable higher order functions it will get more readable and easier to grasp about.
...
```go
func IterateBackward(slice []int) <-chan int {
    ch := make(chan int)
    go func() {
...
This example highlights the current approach to creating iterator-like functionality in Go, showcasing both its utility and the complexity that some developers find challenging.
