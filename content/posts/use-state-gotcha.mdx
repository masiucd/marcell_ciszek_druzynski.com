...
Contrary to what one might expect, calling `setNumber(number + 1) `multiple times in rapid succession within the same click handler does not increment the number state by `3`. This is due to JavaScript creates a closure around the number variables, it’s as if we’re doing:
