...
about: "The Builder pattern is a design pattern used to create objects with complex constructors. It allows for the construction of objects step by step, with each step returning a modified object until the final product is achieved. This pattern is particularly useful when dealing with objects that require a large number of parameters, or when we want to enforce a certain order of operations during object construction."
date: 2023-05-21
updated: 2023-05-21
tags: ["pattern"]
---
...
### Creating a user
Let's say we want to create a `class` that can create user objects with some useful properties like:
- name
- age
- phone
- address
